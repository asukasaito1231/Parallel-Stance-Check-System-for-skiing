...
out = cv2.VideoWriter('output_expand.mp4', fourcc, fps, (width, height))

# 拡大映像保存用の設定
zoom_width = int(width // 2)  # 拡大映像のサイズを設定
zoom_height = int(height // 2)
out_zoom = cv2.VideoWriter('zoomed.mp4', fourcc, fps, (zoom_width, zoom_height))
...
while True:
    ...
    if len(results[0].boxes) > 0:
        ...
        # 拡大表示と保存
        if zoomed_region.size > 0:  # 領域が有効な場合のみ処理
            # 一定サイズにリサイズ
            zoomed_region_resized = cv2.resize(zoomed_region, (zoom_width, zoom_height))
            cv2.imshow('Zoomed Detection', zoomed_region_resized)
            # out_zoom.write(zoomed_region_resized)
